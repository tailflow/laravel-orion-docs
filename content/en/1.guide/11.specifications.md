...
The command will create a `specs.json` file inside the `storage/app/specs` directory.
...
By default, the specifications file is stored in `.json` format. However, it is possible to store it in `.yaml`, simply by providing the `--format` option.
