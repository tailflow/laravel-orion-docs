...
::warning
The order in which query constraints are applied does **not** depend on the order of properties in the payload. It is always defined as follows: `scopes` -> `filters` -> `search` -> `sort` -> `includes` -> `aggregates`.
::
...
It is recommended to use query scopes, because the query constraints are encapsulated in the API (scope method on a model) and no changes to frontend (end-client) would be required, if these constraints change.

Filters, on the other hand, provide a really flexible way of applying query constraints, as if you would do it via the Eloquent query builder.
...
The `type` (default is `and`) property serves as a logical operator for combining multiple filters and can be either `and` or `or`. Under the hood it defines whether to use `where` or `orWhere` method on the query builder for applying a filter.
...
In the request above, the entities would be returned by the API if the `created_at` field is greater than or equal to `2020-01-01` ***OR*** `options->visible` field is equal to `true` ***AND*** `meta.source_id` field value is in array of `[1,2,3]`.
...
In the example above `user.id` and `meta.source_id` are examples of such attributes.

<br/><br/>

Many-to-many relation resources can also be filtered by their pivot values. Just use the `pivot.<field>` notation, where `<field>` is a field on the pivot table.
...
In the example above `options->visible` is one such attribute.
::

## Keyword Search

This type of search is something you would normally do, for example, as a search input functionality on your website to find all blog posts that have a phrase "Laravel is awesome" in them.
First, you need to define the list of fields across which the search will be performed:

::warning
The search is **case-sensitive** by default. You can change this behavior by setting `search.case-sensitive` to `false` in `orion.php` config.
::
...
At the moment, the search is performed using a database query on **all** of the specified fields.
...
In the example above `user.name` is one such attribute.
...
In the example above `options->key` is one such attribute.
::
...
In the example above `meta.priority` is one such attribute.

<br/><br/>

Many-to-many relation resources can also be sorted by their pivot values. Just use the `pivot.<field>` notation, where `<field>` is a field on the pivot table.
...
::warning
Be aware that `count` and `exists` aggregates do not work like other aggregates and require only the `relation` field.
::
...
::warning
Unlike the `include` method, the `alwaysIncludes` method does not support wildcards.
::
