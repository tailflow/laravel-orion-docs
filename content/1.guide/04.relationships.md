...
Many-to-many relation resource provides a `pivot` endpoint to update the pivot row of one of the related models. For details on how pivot row is updated, take a look at [Updating A Record On A Pivot Table](https://laravel.com/docs/eloquent-relationships) section in Laravel Documentation.
...
Just like [model resources](/guide/models#customizing-keys), relation resources are using the primary key to fetch resources from the database.
...
It is possible, same as in [model controllers](/guide/models#customizing-queries), to redefine Eloquent queries for each endpoint. The only major difference is that each endpoint in the relation controller also has "build" and "run" methods for fetching relation's parent model.
...
::callout{icon="i-lucide-lightbulb"}
You can also customize how attributes are filled on the model by overriding the `performFill` method.
::
