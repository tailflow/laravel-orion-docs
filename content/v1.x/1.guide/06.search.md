...
seo:
  title: Search (1.x)
  description: Learn how to perform search, apply filters, and sort results in Orion 1.x. This is the legacy version.
---
...
::warning
The order in which query constraints are applied does **not** depend on the order of properties in the payload. It is always defined as follows: `scopes` -> `filters` -> `search` -> `sort`.
::
