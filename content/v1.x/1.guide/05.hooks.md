---
title: Hooks
description: One of the powerful features of Orion is Hooks. It allows you to tap into the flow inside a particular endpoint (or multiple endpoints) without the need to override a method itself.
seo:
  title: Hooks (1.x)
  description: Learn how to tap into the flow inside an endpoint with hooks in Orion 1.x. This is the legacy version.
---

Here is a common use case: imagine you have an API controller for blog posts and whenever a blog post is created, it needs to be associated with the currently authenticated user. To accomplish that, simply add a `beforeSave` hook and associate a user with the post:
...
* afterIndex - executed after retrieving the list of models, but before building response.
...
* beforeSave - executed before the model is saved, but after `beforeStore` and `beforeUpdate`.
* afterSave - executed after the model is saved, but before `afterStore` and `afterUpdate`.
