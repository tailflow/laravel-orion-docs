...
seo:
  title: Batch Operations (1.x)
  description: Learn how to manage entities in batches in Orion 1.x. This is the legacy version.
---
...
The endpoint expects an object in the request payload with `resources` **array**, where each item is an object representing a resource (e.g. a post) to create.
...
The endpoint expects an object in the request payload with `resources` **object**, where each key is a resource id and each item is an object representing a resource (e.g. a post) to update.
...
The endpoint expects an object in the request payload with `resources` array, where each item is the id of a resource (e.g. a post) to delete.
...
The endpoint expects an object in the request payload with `resources` array, where each item is the id of a resource (e.g. a post) to restore.
