...
seo:
  title: Models (1.x)
  description: Learn how to expose models via API in Orion 1.x. This is the legacy version.
---

## Setting up Controller

To expose a model through the API, first you need to create a controller for it. As you may have seen in the [Getting Started - Simple CRUD](/v1.x/guide/getting-started#simple-crud) section, defining a model controller is pretty straightforward.
...
By default, resources returned by the `index` endpoint are paginated. To disable the pagination and return all resources, you can use the `DisablePagination` trait.
...
* Pagination can be disabled for both model and relation resources using the `DisablePagination` trait

::

## Setting up Routes

Once the controller is created, it is time to register routes.
...
If your model uses the `SoftDeletes` trait and you would like to expose the same functionality via the API, call the `withSoftDeletes` method upon resource registration.
...
Orion is quite flexible and allows you to redefine how Eloquent queries are built and run for each endpoint.
...
In the given example we would force fill the attributes on the post, if the currently authenticated user is an admin (roles implementation here is imaginary):
...
Cool, now the index endpoint returns only published posts. But what if you would like to apply the same constraint to `show` endpoint as well? Sure enough, we could duplicate it and override the `buildShowFetchQuery` method, but there is a better way.

#### Building queries

If you take a look at how `buildIndexFetchQuery` method is implemented, you will notice that it uses the `buildFetchQuery` method. In fact, this method is used by `index`, `show`, `update`, `destroy`, and `restore` endpoints to build the query for fetching model(s), and you can override it as well!

Here is what it would look like if you would like to apply the same constraint to `index` and `show` endpoints (and `update`, `destroy`, and `restore`) at once:
