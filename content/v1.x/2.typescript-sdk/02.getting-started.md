...
seo:
  title: Getting Started (TypeScript SDK 1.x)
  description: Take the first steps to start using Orion TypeScript SDK 1.x. This is the legacy version.
---
...
Once the package is installed, the next step is to configure (or *initialize*) it to work with your API.
...
You do not need to define the `id`, `created_at`, and `updated_at` attributes - the SDK does that for you
...
::warning
If you are using Sanctum, CSRF protection needs to be initialized, before you can query the API. Learn how to accomplish that in the [Sanctum integration guide](/v1.x/typescript-sdk/configuration#integration-with-sanctum-for-spa).
::
